...
import UiSettings, {
  IBrailleSettings,
} from '../components/ui-settings/UiSettings'
  ...
  const handleSendClicked = () => {
    if (!isSerialCompatible()) {
      const alertProps: IUiAlertProps = {
        title: 'ERROR',
        message:
          'Your browser is unfortunatelly not supported. Try new version of MS Edge or Google Chrome.',
        show: true,
        handleClose: () => {
          setAlert({ show: false })
        },
    ...
    sendSerial(
      gcode.split('\n'),
      brailleSettings.baudRate,
      brailleSettings.restrictSerialPort,
      handlePrinterInfo,
      (state: ISerialState) => setPrintState(state)
    )
      .catch((err: Error) => {
        const alertProps: IUiAlertProps = {
          title: 'ERROR',
    ...
    <div>
      <div className="br-ui">
        ...
        <div className="br-ui-controls">
          ...
          />
          <PrintState
            className="br-ui-controls__print-state"
            state={printState}
          />
        </div>
